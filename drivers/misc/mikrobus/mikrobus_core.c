				    ...
				    char *board_name)
{
	...
	if (dev->regulators) {
		...
		dev->regulator_pdev = devm_kzalloc(
			&port->dev, sizeof(uintptr_t) * dev->num_regulators, GFP_KERNEL);
		...
		for (i = 0; i < dev->num_regulators; i++) {
			...
			dev->regulator_pdev[i] = regulator_register_always_on(
				0, dev->regulators[i].name, &regulator, 1, *val);
		}
	}
	...
	case GREYBUS_PROTOCOL_SPI:
		...
		if (dev->clocks) {
			dev->clks = devm_kzalloc(&port->dev, sizeof(uintptr_t) * dev->num_clocks,
						 GFP_KERNEL);
			for (i = 0; i < dev->num_clocks; i++) {
				...
				dev->clks[i] = clk_register_fixed_rate(
					&spi->dev, dev->clocks[i].name, devname, 0, *val);
			}
		}
				       ...
				       char *board_name)
{
	...
	kfree(dev->regulators);
	kfree(dev->clocks);

	devm_kfree(&port->dev, dev->regulator_pdev);
	devm_kfree(&port->dev, dev->clks);
